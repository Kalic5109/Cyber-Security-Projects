...
PEXPECT_LOG_FILE = 'pexpect_log.txt'
...
msf_script = pexpect.spawn('msfconsole', encoding='utf-8', logfile=fout)
...
# Wait for "Meterpreter session * opened" or timeout after 60 seconds
expect_index = msf_script.expect_exact(['Meterpreter session \d+ opened', pexpect.TIMEOUT], timeout=60)
if expect_index == 0:
    # Successfully opened Meterpreter session
    log_info('Meterpreter session opened')
    # Wait for "meterpreter >" prompt
    msf_script.expect_exact('meterpreter >')
elif expect_index == 1:
    # Timeout occurred, press Ctrl+C
    log_info('Timeout occurred, sending Ctrl+C')
    msf_script.sendintr()
    # Wait for "meterpreter >" prompt
    msf_script.expect_exact('meterpreter >')
    log_info('Continuing after timeout')

# Check if Meterpreter session closed
expect_index = msf_script.expect_exact(['meterpreter session \d+ closed', pexpect.TIMEOUT], timeout=10)
if expect_index == 0:
    log_info('Meterpreter session closed')
    # Continue with the next steps
    msf_script.expect_exact('meterpreter >')
elif expect_index == 1:
    log_info('No Meterpreter session closed')

# Check if "auxiliary" prompt appears before the next command
if msf_script.expect(['auxiliary', pexpect.TIMEOUT], timeout=10) == 0:
    # "auxiliary" prompt appeared, send "run" command
    msf_script.sendline('run')
